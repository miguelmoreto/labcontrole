...
def build_fit_matrix(output_vect, input_vect, numorder, denorder):
    ...
    A = numpy.zeros((len(output_vect),numorder+denorder+1))#the +1 accounts
    ...
    for n in range(1, numorder+1):
        curin = numpy.r_[[0.0], curin[0:-1]]#prepend a 0 to curin and drop its
                                    #last element
        A[:,n] = curin
    ...
    for n in range(0, denorder):
        curout = numpy.r_[[0.0], curout[0:-1]]
        A[:,firstden+n] = curout
    return A
...
def prependzeros(num, den):
    ...
    nn = len(num)
    if nn < nd:
        zvect = numpy.zeros(nd-nn)
        numout = numpy.r_[zvect, num]
    else:
        numout = num
    return numout, den
...
def tustin_sub(polyin, T, a=2.0):
    numsub = a*numpy.poly1d([1.0,-1.0])
    densub = T*numpy.poly1d([1.0,1.0])
...
def create_swept_sine_input(maxt, dt, maxf, minf=0.0, deadtime=2.0):
    t = numpy.arange(0, maxt, dt)
    u = sweptsine(t, minf=minf, maxf=maxf)
    if deadtime:
        deadt = numpy.arange(0,deadtime, dt)
        zv = numpy.zeros_like(deadt)
        u = numpy.r_[zv, u, zv]
    return u
...
def ADC(vectin, bits=9, vmax=2.5, vmin=-2.5):
    ...
    dv = (vmax-vmin)/2**bits
    vect2 = numpy.clip(vectin, vmin, vmax)
    counts = vect2/dv
...
#        _inccount()
        ...
        mylines=[]
        for q in range(numpy.shape(matin)[1]):
...
def _checkpoles(poleloc,pnew):
    evect = abs(poleloc-numpy.array(pnew))
    ind = evect.argmin()
    pout = pnew[ind]
    return pout
...
def _realizable(num, den):
    realizable = False
    if not numpy.isscalar(den):        
        if numpy.isscalar(num):
            realizable = True
...
def shape_u(uvect, slope):
    u_shaped = numpy.zeros_like(uvect)
    u_shaped[0] = uvect[0]
...
class TransferFunction(signal.lti):
    ...
    def rlocfind(self, poleloc):
        ...
        k = numpy.optimize.fmin(self.opt,[kinit])[0]
...
##         ax2.semilogx(modelf, mphase)
    ...
    def SimpleFactor(self):
        ...
        snum=mynum/nsf
        poles=sden.r
        residues=numpy.zeros(numpy.shape(sden.r),'D')
        factors=[]
        for x,p in enumerate(poles):
            ...
            tempden=1.0
            for cp in polelist:
                tempden=tempden*(numpy.poly1d([1,-cp]))
            tempTF=TransferFunction(snum,tempden)
            curres=tempTF(mypole)
            residues[x]=curres
            curTF=TransferFunction(curres,numpy.poly1d([1,-mypole]))
    ...
    def PartFrac(self, eps=1.0e-12):
        ...
        while N > 0:
            ...
            if abs(curp.imag) < eps:
                ...
                p1 = numpy.poly1d([1,-curp])
                p2 = numpy.poly1d([1,-pconj])
    ...
    def FreqResp(self, f, fignum=1, fig=None, clear=True, \
        ...
        self.phase = numpy.angle(self.comp,1)
    ...
    def CrossoverFreq(self, f):
       ...
       t1 = numpy.squeeze(self.dBmag > 0.0)
       t2 = numpy.r_[t1[1:],t1[0]]
       t3 = (t1 & -t2)
       myinds = numpy.where(t3)[0]
       if not myinds.any():
          return None, []
       maxind = max(myinds)
    ...
    def PhaseMargin(self,f):
       ...
       if not fc:
          return 180.0
       return 180.0+numpy.squeeze(self.phase[ind])
    ...
    def create_tvect(self, dt=None, maxt=None):
        ...
        if maxt is None:
            ...
            self.maxt = maxt
        tvect = numpy.arange(0,maxt+dt/2.0,dt)
        self.t = tvect
        return tvect
    ...
    def _c2d_sub(self, numsub, densub, scale):
        ...
        for p, coeff in enumerate(self.num.coeffs):
            mynum += numpy.poly1d(coeff*(scale**(m-p))*((numsub**(m-p))*(densub**(n-(m-p)))))
        myden = 0.0
        for p, coeff in enumerate(self.den.coeffs):
            myden += numpy.poly1d(coeff*(scale**(n-p))*((numsub**(n-p))*(densub**(n-(n-p)))))
    ...
    def c2d_tustin(self, dt=None, a=2.0):
        ...
        scale = a/dt
        numsub = numpy.poly1d([1.0,-1.0])
        densub = numpy.poly1d([1.0,1.0])
    ...
    def DigitalSim(self, u, method='zoh', bits=9, vmin=-2.5, vmax=2.5, dt=None, maxt=None, digitize=True):
        ...
        uvect = numpy.zeros(len(B), dtype='d')
        yvect = numpy.zeros(len(A)-1, dtype='d')
        if digitize:
            ...
            udig = u
            dv = 1.0
        Ydig = numpy.zeros(len(u), dtype='d')
        for n, u0 in enumerate(udig):
            uvect = shift(uvect, u0)
            curY = numpy.dot(uvect,B)
            negpart = numpy.dot(yvect,A[1:])
            curY -= negpart
            if digitize:
                curY = int(curY)
            Ydig[n] = curY
...
class Digital_PI(object):
   ...
   def prep(self):
      self.esum = numpy.zeros_like(self.input)
...
class Digital_P_Control(Digital_Compensator):
   def __init__(self, kp, input_vect=None, output_vect=None):
      ...
      self.input = input_vect
      self.output = output_vect
      self.num = numpy.poly1d([kp])
      self.den = numpy.poly1d([1])
      self.gain = 1
...
class FirstOrderCompensator(Compensator):
   def __init__(self, K, z, p, dt=0.004):
      ...
      D(s) = -----------
                (s+p)    """
      Compensator.__init__(self, K*numpy.poly1d([1,z]), [1,p])
...
class Closed_Loop_System_with_Sat(object):
   ...
   def lsim(self, u, t, X0=None, include_sat=True, \
      ...
      dt = t[1]-t[0]
      if X0 is None:
         X0 = numpy.zeros((2,len(self.plant_tf.den.coeffs)-1))
      N = len(t)
      y = numpy.zeros(N)
      v = numpy.zeros(N)
      x_n = X0
...
def Usweep(ti,maxt,minf=0.0,maxf=10.0):
    ...
    if ti<0.0:
        ...
        if ti<(maxt*0.95):
            return numpy.sin(2*numpy.pi*curf*ti)
        else:
            return 0.0
